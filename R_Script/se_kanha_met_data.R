...
# -----------------------------
roi_shp_path  <- "D:\\NASA_Power_Kanha_Metdata_2020_2021\\ROI\\South_East\\kanha_south_east.shp"
ndvi_csv_path <- "D:\\NASA_Power_Kanha_Metdata_2020_2021\\Data_Table\\data_ndvi\\se_kanha_table_5day_interpolated_SG.csv"
...
centroid <- st_centroid(st_union(roi))
...
match_nearest_met <- function(ndvi_date, met_data, max_days = 5) {
  ...
  # If no met data within ±5 days, return NA row
  if (nrow(met_sub) == 0) {
    return(tibble(
      date = ndvi_date,
      T2M = NA_real_,
